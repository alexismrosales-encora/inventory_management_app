...
public class InventoryService implements IInventoryService {
   ...
   * Converts the given InventoryDTO to an Inventory entity, saves it using the
   * repository,
   ...
   * Fetches inventory items from the repository using the provided pagination
   * request,
  ...
  public List<InventoryDTO> getInventory(PaginationRequestDTO paginationRequestDTO) {
    ...
    return inventory.stream().map((inventoryItem) -> InventoryMapper.mapToInventoryDto(inventoryItem))
        .collect(Collectors.toList());
  }

  /*
   * Updates an existing product.
   * Converts the given InventoryDTO to an Inventory entity, updates it using the
   * repository,
   ...
   * Retrieves the product by its ID, sets its stock status to OUT_OF_STOCK and
   * quantity to 0,
   ...
   * Retrieves the product by its ID, sets its stock status to IN_STOCK and
   * quantity to 10,
   * updates it using the repository, and returns the updated InventoryDTO.
   *
   * @param id the ID of the product to update
  ...
  public MetricsDTO calculateMetrics() {
    ...
    for (String category : categories) {
      ...
      CategoryMetric categoryMetric = new CategoryMetric(category, totalProductsInStock, totalValueInStockByCategory,
          averagePriceInStockByCategory);
