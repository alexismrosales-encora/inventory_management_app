...
import com.breakabletoy.ima_backend.service.IInventoryService;
...
public class InventoryController {
  ...
  /*
   * GET /api/products
   * Retrieves a paginated list of inventory items based on filter, sorting, and
   * pagination parameters.
   * Expects optional parameters for sorting, filtering by stock status,
   * categories, and search text.
   * Returns a PaginatedResponse containing the list of inventory items and total
   * count.
   */
  @GetMapping()
  public ResponseEntity<PaginatedResponse> findAllInventoryItems(@RequestParam(defaultValue = "0") int page,
    ...
    savedPaginationRequestDTO = new PaginationRequestDTO(page, size, sortBy, sortOrder, stockStatus, categoriesParam,
        searchParam);
   ...
   * Expects the inventory item ID as a path variable and updated data in the
   * request body.
   ...
   * Retrieves inventory metrics such as total value, average price, and category
   * metrics.
